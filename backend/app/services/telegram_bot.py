...
async def ensure_webhook() -> None:
    ...
    if webhook_url and not settings.bot_webhook_url:
        derived_from = "PUBLIC_BASE_URL" if settings.public_base_url else "RENDER_EXTERNAL_URL"
        logger.info(
            "Derived Telegram webhook URL from %s: %s", derived_from, webhook_url
        )

    if not webhook_url:
        logger.warning(
            "No BOT_WEBHOOK_URL, PUBLIC_BASE_URL or RENDER_EXTERNAL_URL configured; Telegram bot may not receive updates."
        )
    ...
    if await set_webhook(webhook_url):
